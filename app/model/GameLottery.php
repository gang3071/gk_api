...
class GameLottery extends Model
{
    ...
    /**
     * 清除游戏彩金相关缓存
     * 注意：缓存键需要与 GameLotteryServices 类中的常量保持一致
     * - CACHE_KEY_LOTTERY_POOL = 'game_pool_3'
     * - CACHE_KEY_LOTTERY_LIST = 'game_lottery_list'
     */
    private static function clearGameLotteryCache(): void
    {
        try {
            \support\Cache::delete('game_pool_3');
            \support\Cache::delete('game_lottery_list');
        } catch (\Exception $e) {
            // 日志记录但不抛出异常，避免影响主流程
            \support\Log::channel('game_lottery')->warning('清除游戏彩金缓存失败', [
                'error' => $e->getMessage()
            ]);
        }
    }

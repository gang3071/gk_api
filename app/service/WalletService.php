...
class WalletService
{
    ...
    private static function atomicDeduct(int $playerId, float $amount): float
    {
        ...
        try {
            $result = \support\Redis::eval($script, 1, $cacheKey, $amount);
...
-- 🔥 精度修正：四舍五入到 2 位小数（消除浮点运算产生的 ±0.00001 级别误差）
-- 原理：将金额放大 100 倍 → 四舍五入到整数 → 缩小 100 倍
-- 示例：
--   4999.9999999 → floor(499999.9999 + 0.5) / 100 = 5000.00 ✅ 修正为整数
--   4999.99      → floor(499999 + 0.5) / 100 = 4999.99 ✅ 保持不变（真实的 99 分）
--   5000.0000001 → floor(500000.0001 + 0.5) / 100 = 5000.00 ✅ 修正为整数
-- 作用：避免因精度问题导致 5000 分只洗出 4900 分
currentBalance = math.floor(currentBalance * 100 + 0.5) / 100

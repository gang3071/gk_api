...
class ForceFixLotteryTicketRecordUniqueIndex extends AbstractMigration
{
    public function up()
    {
        ...
        $hasNewIndex = false;
        try {
            $rows = $this->fetchAll("SHOW INDEX FROM yjb_lottery_ticket_record WHERE Key_name = 'idx_activity_ticket_no_unique'");
        ...
        if (!$hasNewIndex) {
            $this->execute("ALTER TABLE yjb_lottery_ticket_record ADD UNIQUE INDEX idx_activity_ticket_no_unique (activity_id, ticket_no)");
    ...
    public function down()
    {
        ...
        // 删除联合索引
        try {
            $this->execute("ALTER TABLE yjb_lottery_ticket_record DROP INDEX idx_activity_ticket_no_unique");
            echo "✓ 删除联合索引\n";
        } catch (\Exception $e) {
            echo "ℹ️ 联合索引不存在\n";
        }
        ...
        // 恢复单字段索引
        try {
            $this->execute("ALTER TABLE yjb_lottery_ticket_record ADD UNIQUE INDEX idx_ticket_no_unique (ticket_no)");
            echo "✓ 恢复单字段索引\n";
        } catch (\Exception $e) {
            echo "ℹ️ 单字段索引已存在\n";
        }

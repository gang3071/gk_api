...
/**
 * 为所有渠道添加高分广播配置
 * 当玩家在第三方游戏中得分超过配置阈值时全频道广播该消息
 */
class AddHighScoreBroadcastSetting extends AbstractMigration
{
    /**
     * 执行迁移
     */
    public function up()
    {
        ...
        foreach ($channels as $channel) {
            ...
            $exists = $this->fetchRow("
                ...
                WHERE department_id = {$departmentId}
                  AND feature = 'high_score_broadcast_threshold'
            ");
            ...
            $this->execute("
                INSERT INTO system_setting (
                    ...
                    {$departmentId},
                    'high_score_broadcast_threshold',
                    {$defaultThreshold},
                    '',
                    NULL,
                    NULL,
            ...
            $this->output->writeln("   ✅ 渠道 {$departmentId} ({$channelName}) - 已创建（阈值: {$defaultThreshold} 分，状态: 禁用）");
            $createdCount++;
        }
        ...
        $this->output->writeln('  1. 访问后管系统设置页面');
        $this->output->writeln('  2. 找到【高分广播阈值】配置项');
    ...
    public function down()
    {
        ...
        $this->output->writeln('   ✅ 已删除所有高分广播配置');
    }
}
